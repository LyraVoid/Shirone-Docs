...
文章列表按发布时间倒序排列。主要控制手段：

- **置顶**：`pinned: true` 排到列表最前（多个置顶之间仍按发布时间倒序）
- **同日多篇排序**：同一天发布多篇文章时，通过带时分秒的时间戳或 `publishedAt` 字段明确先后顺序（缺省时按文章 ID 排序）
- **补录旧文**：把 `published` 写成实际日期即可归位，无需迁移文件

> [!TIP] 精确时刻控制
> 纯日期（`YYYY-MM-DD`）适合单日单篇发布；若需精准控制同日多篇发布顺序，建议在 Frontmatter 中提供完整时间戳（如 `2026-08-26T14:30:00+08:00`）或使用 `publishedAt` 字段，详见 [Frontmatter 时间与时区](/guide/frontmatter/#时间与时区)。
