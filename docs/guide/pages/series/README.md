...
| 体验形态 | 所在位置 / 网址 | 特点与交互说明 |
| --- | --- | --- |
| **文章内连载导航** | 正文底部（或顶部） | 自动标明所属系列名称、状态小标签（进行中 / 已完结）、当前是第几篇（如「第 2 / 共 5 篇」），并提供「上一篇」与「下一篇」的快速翻页跳转。 |
| **全站系列总览页** | `/series/` | 以全宽卡片整齐罗列站内所有的系列专栏，展示系列封面导读、文章篇数以及最新更新时间，方便访客发现感兴趣的专题。 |
| **系列详情目录页** | `/series/<系列标识>/` | 突出展示该系列的专属导读前言，并按照预设顺序整齐列出属于该系列的所有文章章节，方便读者系统化按序阅读。 |
