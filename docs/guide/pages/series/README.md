...
:::: card-grid cols="3"
::: card title="1. 文章内连载导航" icon="ri:compass-3-line"
正文底部（或顶部）自动展示连载导航区，标出系列名称、状态小标签（进行中/已完结）、当前篇目进度（如「第 2/共 5 篇」），并提供上一篇与下一篇的快速跳转。
:::

::: card title="2. 全站系列总览 (/series/)" icon="ri:layout-grid-line"
全宽精美卡片整齐排列所有专栏，展示系列封面导读、文章篇数与最新更新时间，方便访客发现感兴趣的专题。
:::

::: card title="3. 系列详情目录 (/series/[slug]/)" icon="ri:list-check-2"
突出展示该系列的专属前言与导读卡片，并按设定顺序罗列该系列包含的全部章节列表，方便读者系统化阅读。
:::
::::
